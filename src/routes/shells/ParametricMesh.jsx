...
export default function ParametricMesh() {
    ...
    const c = useControls("Generating Curve", {
        b: { value: 0.01, min: 0, max: 10, step: 0.01 },
        d: { value: 1, min: 0, max: 10, step: 0.01 },
    ...
    const g = useControls("Geometry Settings", {
        ...
        s: { value: 11, min: 0, max: 50, step: 1 },
        wireframe: false
    })
    ...
    const curve = getCurvePoints(e.a, c.b, d.c, g.detail, c.z, c.d, g.s, g.sDetail, e.phi, d.n, d.cDepth, d.nDepth)
...
function getCurvePoints(a, b, c, detail, z, d, s, sDetail, phi, n, cDepth, nDepth) {
    let curve = []
    ...
    for (let i = 0.1; i < s; i += Math.PI / detail) {
        ...
        for (let j = 0; j <= sDetail; j++) {
            ...
            let point = new THREE.Vector3(x * i * nCurv * ic, 0, z * i * nCurv * ic)
