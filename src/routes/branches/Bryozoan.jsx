...
import PlanarBryozoan from '../../markdown/PlanarBryozoan.md'

export default function Bryozoan() {
    const [text, setText] = useState('')
    useEffect(() => {
        fetch(PlanarBryozoan).then(res => res.text()).then(text => setText(text))
    })
...
function getCurvePoints(a, ar, b, br, d) {
    ...
    for (let i = 0; i < d; i++) {
        ...
        for (let p of layer) {
            const al = ca / 2 + p.w
            const ar = -ca / 2 + p.w
            const l = new THREE.Vector4(cb * Math.sin(al) + p.x, cb * Math.cos(al) + p.y, p.z, al)
            const r = new THREE.Vector4(cb * Math.sin(ar) + p.x, cb * Math.cos(ar) + p.y, p.z, ar)
            newLayer.push(l, r)
            curve.push([p, l], [p, r])
        }
        layer = newLayer
